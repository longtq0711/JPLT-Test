...
/***/ "./app/Modules/Admin/Resources/js/campaigns/index.js":
/*!***********************************************************!*\
  !*** ./app/Modules/Admin/Resources/js/campaigns/index.js ***!
  \***********************************************************/
/*! no exports provided */
...
new Vue({
  ...
  created: function created() {
    this.addCampaign = this.defaultCampaign();
    this.suppliers = JSON.parse($('#suppliers').val());
    this.categories = JSON.parse($('#categories').val());
    this.products = JSON.parse($('#products').val());
    this.campaigns = JSON.parse($('#campaigns').val());

    if (this.campaigns.length > 0) {
      this.campaignCurrent = this.campaigns[0];
    }
  },
  methods: {
    defaultCampaign: function defaultCampaign() {
      return {
        id: null,
        code: null,
        name: null,
    ...
    storeCamp: function storeCamp() {
      this.addErrors = [];

      if (!this.addCampaign.code) {
        this.addErrors['code'] = 'Vui lòng nhập mã chiến dịch';
      }

      if (!this.addCampaign.name) {
        this.addErrors['name'] = 'Vui lòng nhập tên chiến dịch';
      }

      if (!this.addCampaign.location) {
        this.addErrors['location'] = 'Vui lòng nhập vị trí chiến dịch';
      }
      ...
      if (Object.keys(this.addErrors).length == 0) {
        ...
        axios({
          method: 'POST',
          url: '/admin/campaigns',
          data: scop.addCampaign
        }).then(function (response) {
          var data = response.data;

          if (data.status === 200) {
            response.data.campaign.products = [];
            scop.campaigns.push(response.data.campaign);
            scop.campaignCurrent = scop.campaigns[scop.campaigns.length - 1];
            scop.addCampaign = scop.defaultCampaign();
            ...
            if (data.datas.code) {
              scop.addErrors['code'] = data.datas.code[0];
            }

            if (data.datas.name) {
              scop.addErrors['name'] = data.datas.name[0];
            }
          }
    ...
    deleteCampaign: function deleteCampaign() {
      var scop = this;
      Swal.fire({
        title: 'Bạn chắc chắn muốn xóa chiến dịch?',
        type: 'warning',
        ...
        if (result.value) {
          ...
          axios({
            method: 'DELETE',
            url: '/admin/campaigns/' + scop.campaignCurrent['id']
          }).then(function (response) {
            var data = response.data;
            ...
            if (data.status === 200) {
              ...
              var index = scop.campaigns.indexOf(scop.campaignCurrent);
              scop.campaigns.splice(index, 1);
              scop.campaignCurrent = scop.campaigns[index - 1];
            }
    ...
    addProduct: function addProduct() {
      var scop = this;
      scop.selected.forEach(function (select) {
        var checkProduct = false;
        scop.campaignCurrent.campaign_products.forEach(function (item) {
          if (item.product_id == select.id) {
            checkProduct = true;
            return;
          }
        });
        ...
        if (!checkProduct) {
          scop.campaignCurrent.campaign_products.unshift({
    ...
    update: function update() {
      ...
      if (!this.campaignCurrent.code) {
        this.updateErrors['code'] = 'Vui lòng nhập mã chiến dịch';
      }

      if (!this.campaignCurrent.name) {
        this.updateErrors['name'] = 'Vui lòng nhập tên chiến dịch';
      }

      if (!this.campaignCurrent.location) {
        this.updateErrors['location'] = 'Vui lòng nhập vị trí chiến dịch';
      }
      ...
      if (Object.keys(this.updateErrors).length == 0) {
        ...
        axios({
          method: 'POST',
          url: '/admin/campaigns/update/' + scop.campaignCurrent.id,
          data: scop.campaignCurrent
        }).then(function (response) {
          if (response.data.status === 200) {
            scop.campaigns.push(response.data.campaignCurrent);
            scop.campaigns.pop();
            Swal.fire({
              icon: 'success',
              title: '',
    ...
    searchProduct: function searchProduct() {
      var scop = this;
      scop.statusSearch = true;
      axios.get('/admin/campaigns/search-product', {
        params: scop.productSearch
      }).then(function (response) {
        scop.statusSearch = false;

        if (response.data.status === 200) {
          scop.products = response.data.products;
        }
      })["catch"](function (error) {
        Swal.fire({
          icon: 'error',
    ...
    selectAll: function selectAll() {
      ...
      if (scop.allSelected == false) {
        scop.products.forEach(function (element) {
          scop.selected.push(element);
        });
...
/***/ 15:
/*!*****************************************************************!*\
  !*** multi ./app/Modules/Admin/Resources/js/campaigns/index.js ***!
  \*****************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\xampp\htdocs\Laginza\mua_ho\app\Modules\Admin\Resources\js\campaigns\index.js */"./app/Modules/Admin/Resources/js/campaigns/index.js");
