...
/***/ "./app/Modules/Admin/Resources/js/products/form.js":
/*!*********************************************************!*\
  !*** ./app/Modules/Admin/Resources/js/products/form.js ***!
  \*********************************************************/
/*! no exports provided */
...
/* harmony import */ var _admin_stores_storeProduct__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @admin/stores/storeProduct */ "./app/Modules/Admin/Resources/js/stores/storeProduct.js");
/* harmony import */ var _core_helpers_product__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @core/helpers/product */ "./resources/js/helpers/product.js");
...
new Vue({
  ...
  created: function created() {
    ...
    var product = JSON.parse($('#product').val()),
        datas = JSON.parse($('#datas').val());

    if (product) {
      this.product = product;

      if (this.product.is_profit == 1) {
        this.product.profit = this.numberToCurrency(product.profit, false, true);
      }

      if (this.product.is_exchange_rate == 1) {
        this.product.exchange_rate = this.numberToCurrency(product.exchange_rate, false, true);
      }
    }

    if (datas.suppliers) {
      this.suppliers = datas.suppliers;
    }
    ...
    if (this.routeType == 'create') {
      this.tab = 'translation';
      this.product.id = datas.product_current_id;
    } else {
      if (datas.attributes) {
        this.attributes = datas.attributes;
      }

      if (datas.productAttributes) {
        this.productAttributes = datas.productAttributes;
      }
      ...
      if (datas.productOriginal) {
        this.productOriginal = datas.productOriginal;
      } else {
        this.tab = 'translation';
      }
      ...
      if (datas.freight) {
        ...
        this.freight.str_price = this.numberToCurrency(datas.freight.price, false, true);
      }

      if (datas.skuNumberStart) {
        this.skuNumberStart = parseInt(datas.skuNumberStart);
      } // this.reloadProductAttributes();


      this.onKeyupMoney(true);
    }

    if (this.attributes.length == 0 || this.productPrice.sku == null) {
      this.productPrice.sku = this.product.id + '-1';
    }
  },
  computed: {
    productSetting: function productSetting() {
      return Object(_core_helpers_product__WEBPACK_IMPORTED_MODULE_2__["getProductSetting"])(this.product, this.globalData.productSetting);
    },
    supplier: function supplier() {
      var data = {};
      var scop = this;
      scop.suppliers.forEach(function (item) {
        if (scop.product.supplier_id == item.id) {
          data = item;
          return;
        }
      });
  ...
  methods: {
    checkChangeField: function checkChangeField(field) {
      if (this.productOriginal && this.productOriginal.field_changes) {
        var field_changes = this.productOriginal.field_changes.split(",");
        var check = false;
        field_changes.forEach(function (element) {
          if (element == field) {
            check = true;
            return;
          }
    ...
    checkProductAttributes: function checkProductAttributes(key) {
      var scop = this,
          check = false,
          data = {};
      $.each(scop.productAttributes, function (i, attribute) {
        $.each(attribute['values'], function (j, attribute_1) {
          if (attribute_1.key == key) {
            check = true;
            data = attribute_1;

            if (!data.sku) {
              data.sku = scop.product.id + '-' + scop.skuNumberStart;
              scop.skuNumberStart++;
            }
    ...
    reloadProductAttributes: function reloadProductAttributes() {
      var scop = this,
          productAttributes = [];

      if (scop.attributes.length > 0) {
        $.each(scop.attributes[0]['values'], function (i, attribute) {
          var values = [];

          if (scop.attributes.length == 1) {
            var checkData = scop.checkProductAttributes(i);

            if (checkData) {
              checkData.name = null;
              values.push(checkData);
            } else {
              values.push({
                key: i,
                id: null,
                name: null,
                weight: scop.priceSetting.weight,
                freight: 0,
                // original_input_price_jp: null,
                input_price_jp: scop.priceSetting.input_price_jp,
                // input_price_vi: scop.priceSetting.input_price_vi,
                // original_sale_price_jp: null,
                sale_price_jp: scop.priceSetting.sale_price_jp,
                // sale_price_vi: scop.priceSetting.sale_price_vi,
                quantity: scop.priceSetting.quantity,
                sku: scop.product.id + '-' + scop.skuNumberStart
              });
              scop.skuNumberStart++;
            }
          } else {
            $.each(scop.attributes[1]['values'], function (j, attribute_1) {
              var checkData = scop.checkProductAttributes(i + '_' + j);

              if (checkData) {
                checkData.name = attribute_1.value;
                values.push(checkData);
              } else {
                values.push({
                  key: i + '_' + j,
                  name: attribute_1.value,
                  weight: scop.priceSetting.weight,
                  freight: 0,
                  // original_input_price_jp: null,
                  input_price_jp: scop.priceSetting.input_price_jp,
      ...
      scop.productAttributes = productAttributes;
    },
    applySettingPrice: function applySettingPrice() {
      var scop = this;
      $.each(scop.productAttributes, function (i, attribute) {
        $.each(attribute['values'], function (j, attribute_1) {
          if (scop.priceSetting.weight) {
            attribute_1.weight = scop.priceSetting.weight;
          }

          if (scop.priceSetting.input_price_jp) {
            attribute_1.input_price_jp = scop.priceSetting.input_price_jp;
          }

          if (scop.priceSetting.input_price_vi) {
            attribute_1.input_price_vi = scop.priceSetting.input_price_vi;
          }

          if (scop.priceSetting.sale_price_jp) {
            attribute_1.sale_price_jp = scop.priceSetting.sale_price_jp;
          }

          if (scop.priceSetting.sale_price_vi) {
            attribute_1.sale_price_vi = scop.priceSetting.sale_price_vi;
          }

          if (scop.priceSetting.quantity) {
            attribute_1.quantity = scop.priceSetting.quantity;
          }
        });
      });
      scop.onKeyupMoney();
    },
    changeCategory: function changeCategory(freight) {
      if (freight) {
        this.freight = freight;
        this.freight.str_price = this.numberToCurrency(freight.price, false, true);
      }

      this.onKeyupMoney(true);
    },
    changeSettingPrice: function changeSettingPrice() {
      if (this.product.is_profit) {
        if (this.product.profit) {
          this.product.profit = this.numberToCurrency(this.product.profit, false, true);
        } else {
          this.product.profit = this.globalData.productSetting.profit;
        }
      }

      if (this.product.is_exchange_rate) {
        if (this.product.exchange_rate) {
          this.product.exchange_rate = this.numberToCurrency(this.product.exchange_rate, false, true);
        } else {
          this.product.exchange_rate = this.globalData.productSetting.exchange_rate;
        }
      }

      this.onKeyupMoney(true);
    },
    onKeyupMoney: function onKeyupMoney() {
      var blur = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;

      if (this.attributes.length > 0) {
        var data = Object(_core_helpers_product__WEBPACK_IMPORTED_MODULE_2__["reloadPriceAttributes"])(this.productAttributes, this.productSetting, this.freight);
        this.productAttributes = [];
        this.productAttributes = data;
      } else {
        var _data = Object(_core_helpers_product__WEBPACK_IMPORTED_MODULE_2__["getProductPrice"])(this.productPrice, this.productSetting, this.freight);

        this.productPrice = {};
        this.productPrice = _data;
      }
    },
    addAttribute: function addAttribute() {
      ...
      this.skuNumberStart = 1;
      this.reloadProductAttributes();
    },
    addAttributeValue: function addAttributeValue(index) {
    ...
    removeAttributeValue: function removeAttributeValue(index, i) {
      this.attributes[index]['values'].splice(i, 1);
      this.reloadProductAttributes();
    },
    titleToSlug: function titleToSlug() {
      this.product.slug = stringToSlug(this.product.title);
    },
    checkValid: function checkValid() {
      var scop = this;
      scop.errors = [];

      if (scop.tab == 'original') {
        if (!scop.productOriginal.status) {
          scop.errors['original_status'] = 'Vui lòng chọn trạng thái';
        }
      } else {
        if (!scop.product.title) {
          scop.errors['title'] = 'Vui lòng nhập tên sản phẩm';
        } // if (!scop.product.slug) {
        //     scop.errors['slug'] = 'Vui lòng nhập slug';
        // }


        if (!scop.product.status) {
          scop.errors['status'] = 'Vui lòng chọn trạng thái';
        }

        if (!scop.product.supplier_id) {
          scop.errors['supplier_id'] = 'Vui lòng chọn thương hiệu';
        }

        if (scop.category_id) {
          scop.errors['category_id'] = 'Vui lòng chọn loại sản phẩm';
        }

        if (!scop.product.avatar) {
          scop.errors['avatar'] = 'Vui lòng chọn ảnh đại diện';
        }

        if (!scop.product.where_product) {
          scop.errors['where_product'] = 'Vui lòng chọn vị trí sản phẩm';
        }

        if (!scop.product.origin) {
          scop.errors['origin'] = 'Vui lòng chọn xuất xứ';
        } // if (!scop.product.original_link) {
        //     scop.errors['original_link'] = 'Vui lòng nhập link gốc';
        // }


        if (scop.product.is_profit && !scop.product.profit) {
          scop.errors['profit'] = 'Vui lòng nhập Tỷ suất lợi nhuận';
        }

        if (scop.product.is_exchange_rate && !scop.product.exchange_rate) {
          scop.errors['exchange_rate'] = 'Vui lòng nhập Tỷ giá quy đổi';
        }

        if (!scop.product.content) {
          scop.errors['content'] = 'Vui lòng nhập nội dung';
        }

        if (scop.attributes.length == 0) {
          // if (scop.productPrice.weight == null) {
          //     scop.errors['weight'] = 'Vui lòng nhập cân nặng';
          // }
          if (scop.productPrice.input_price_jp == null) {
            scop.errors['input_price_jp'] = 'Vui lòng nhập giá tiền';
          } // if (scop.productPrice.input_price_vi == null) {
          //     scop.errors['input_price_vi'] = 'Vui lòng nhập giá tiền';
          // }


          if (scop.productPrice.sale_price_jp == null) {
            scop.errors['sale_price_jp'] = 'Vui lòng nhập giá tiền';
          } // if (scop.productPrice.sale_price_vi == null) {
          //     scop.errors['sale_price_vi'] = 'Vui lòng nhập giá tiền';
          // }

        } else {
          // check validate attribute
          $.each(scop.attributes, function (i, attribute) {
            if (attribute.name == null || attribute.name == "") {
              scop.errors['attribute'] = {};
              scop.errors['attribute'][i] = 'Vui lòng nhập tên thuộc tính';
              return;
            }

            if (!scop.errors['attribute']) {
              $.each(attribute['values'], function (j, value) {
                if (value.value == null || value.value == "") {
                  scop.errors['attribute'] = {};
                  scop.errors['attribute'][i] = 'Vui lòng nhập đầy đủ giá trị thuộc tính';
                  return;
                }
              });
            }
          ...
          }); // validate trùng tên thuộc tính

          if (scop.attributes.length > 1) {
            if (!scop.errors['attribute'] && scop.attributes[0].name.toLowerCase() == scop.attributes[1].name.toLowerCase()) {
              scop.errors['attribute'] = {};
              scop.errors['attribute'][0] = 'Tên thuộc tính không được giống nhau';
            }
          } // Validate trùng lặp giá trị thuộc tính


          if (!scop.errors['attribute']) {
            $.each(scop.attributes, function (i, attribute) {
              $.each(attribute['values'], function (j, value) {
                $.each(attribute['values'], function (k, value1) {
                  if (j != k && value.value.toLowerCase() == value1.value.toLowerCase()) {
                    scop.errors['attribute'] = {};
                    scop.errors['attribute'][i] = 'Giá trị thuộc tính không được giống nhau';
                    return;
                  }
                });
          ...
          $.each(scop.productAttributes, function (i, pa) {
            $.each(pa['values'], function (i, vl) {
              if (vl.input_price_jp == null || vl.sale_price_jp == null || vl.quantity == null) {
                scop.errors['product_attribute'] = 'Vui lòng nhập đầy đủ giá tiền và số lượng';
                return;
              }
            });

            if (scop.errors['product_attribute']) {
              return;
            }
          });
        }
    ...
    handleSubmit: function handleSubmit() {
      ...
      if (scop.tab == 'original' && scop.product.id) {
        scop.$loading(true);
        _admin_stores_storeProduct__WEBPACK_IMPORTED_MODULE_1__["default"].dispatch('updateOriginal', {
          product_id: scop.productOriginal.id,
          data: {
            productOriginal: scop.productOriginal
          }
        }).then(function (response) {
          if (response.status === 200) {
            if (scop.productOriginal.status == 'translated') {
              scop.productOriginal.field_changes = null;
            }
        ...
        if (Object.keys(scop.errors).length == 0) {
          ...
          _admin_stores_storeProduct__WEBPACK_IMPORTED_MODULE_1__["default"].dispatch('save', {
            method: method,
            url: scop.submitUrl,
            data: {
              product: scop.product,
              productImages: scop.productImages,
              attributes: scop.attributes,
              productAttributes: scop.productAttributes,
              productPrice: scop.productPrice
            }
          }).then(function (response) {
            if (response.status === 200) {
              Swal.fire({
              ...
              if (response.datas.title) {
                scop.errors['title'] = response.datas.title[0];
              }

              if (response.datas.slug) {
                scop.errors['slug'] = response.datas.slug[0];
              }

              if (response.datas.status) {
                scop.errors['status'] = response.datas.status[0];
              }

              if (response.datas.supplier_id) {
                scop.errors['supplier_id'] = response.datas.supplier_id[0];
              }

              if (response.datas.category_id) {
                scop.errors['category_id'] = response.datas.category_id[0];
              }

              if (response.datas.avatar) {
                scop.errors['avatar'] = response.datas.avatar[0];
              }

              if (response.datas.where_product) {
                scop.errors['where_product'] = response.datas.where_product[0];
              }

              if (response.datas.origin) {
                scop.errors['origin'] = response.datas.origin[0];
              }

              if (response.datas.original_link) {
                scop.errors['original_link'] = response.datas.original_link[0];
              }
            } else {
              Swal.fire({
                icon: 'error',
...
/***/ 10:
/*!***************************************************************!*\
  !*** multi ./app/Modules/Admin/Resources/js/products/form.js ***!
  \***************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\xampp\htdocs\Laginza\mua_ho\app\Modules\Admin\Resources\js\products\form.js */"./app/Modules/Admin/Resources/js/products/form.js");
