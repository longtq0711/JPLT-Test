...
INSERT INTO `tests` (`id`, `title`, `description`, `slug`, `finished_at`, `status`, `created_at`, `updated_at`) VALUES
...
--
ALTER TABLE `questions`
  ADD CONSTRAINT `questions_quiz_id_foreign` FOREIGN KEY (`quiz_id`) REFERENCES `tests` (`id`) ON DELETE CASCADE;

--
-- Tablo kısıtlamaları `results`
--
ALTER TABLE `results`
  ADD CONSTRAINT `results_quiz_id_foreign` FOREIGN KEY (`quiz_id`) REFERENCES `tests` (`id`) ON DELETE CASCADE,
